...
#define BLE_UUID_NUS_BATTERY_CHARACTERISTIC 0x0004                      /**< The UUID of the RX Characteristic. */
#define BLE_UUID_NUS_CONTROL_CHARACTERISTIC 0x0005                      /**< The UUID of the RX Characteristic. */
...
static void on_write(ble_nus_t * p_nus, ble_evt_t * p_ble_evt)
{
    ...
    if (
        ...
        &&
        (p_evt_write->len == 2)
       ) // 使能notify，ble_nus_string_send得以发出notify
    {
        ...
        else
        {
            ...
            ) // 板子收到手机来的数据，用data_handler通过串口发出
    {
        p_nus->data_handler(p_nus, p_evt_write->data, p_evt_write->len);
    }
    else if (p_evt_write->handle == p_nus->control_handles.value_handle)
    {
		if(p_nus->control_handler != NULL)
			p_nus->control_handler(p_nus, p_evt_write->data, p_evt_write->len);
...
static uint32_t battery_char_add(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
    ...
    ble_uuid.type = p_nus->uuid_type;
    ble_uuid.uuid = BLE_UUID_NUS_BATTERY_CHARACTERISTIC;
 ...
 * @return NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t oled_char_add(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_t    attr_char_value;
    ble_uuid_t          ble_uuid;
    ...
    ble_uuid.type = p_nus->uuid_type;
    ble_uuid.uuid = BLE_UUID_NUS_CONTROL_CHARACTERISTIC;
...
uint32_t ble_nus_init(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
    ...
    p_nus->control_handler			= p_nus_init->control_handler;
    p_nus->is_notification_enabled = false;
    ...
    // Add the TX Characteristic.
    err_code = oled_char_add(p_nus, p_nus_init);
    if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
